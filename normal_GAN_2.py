...
if __name__ == '__main__':
    ...
    G_Optim = Adam(G.parameters(), 0.0004, betas=(0.5, 0.999))
    ...
    summary = tensorboardX.SummaryWriter('./normal_GAN_log_2')
    for epoch in range(500):
        ...
        for index in range(0, data.shape[0], batch_size):
            ...
            D_fake_label = torch.zeros(z.size(0), 1) + 0.3  # torch.Tensor(real.size(0), 1).uniform_(0, 0.3)
            D_real_label = torch.ones(z.size(0), 1) - 0.3  # torch.Tensor(real.size(0), 1).uniform_(1 - 0.3, 1)
            G_fake_label = torch.ones(z.size(0), 1) - 0.3  # torch.Tensor(real.size(0), 1).uniform_(1 - 0.3, 1)
        ...
        torch.save(G.state_dict(), 'params_G_2.pkl')
        torch.save(D.state_dict(), 'params_D_2.pkl')
    summary.close()
